...
function evaluateSlotMachine(Arr) {
  ...
  var unique = Arr.filter((v,i,a)=>a.indexOf(v)===i).length;
  var sum = Arr.reduce((sum,a)=> parseInt(sum)+parseInt(a));
  if (unique === 3) return "YOU LOSE "+sum*50+' DOLLARS, YOUR TOTAL CREDIT BALANCE IS '+ (creditBalance - (sum*50)) +' DOLLARS';
  else if (unique === 2) return "YOU WIN "+sum*100+' DOLLARS, YOUR TOTAL CREDIT BALANCE IS '+(creditBalance + (sum*100)) +' DOLLARS';
  else  return "YOU WIN "+sum*200+' DOLLARS, YOUR TOTAL CREDIT BALANCE IS '+(creditBalance+(sum*200))+' DOLLARS';
